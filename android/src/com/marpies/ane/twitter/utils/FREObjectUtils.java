...
public class FREObjectUtils {
	...
	public static List<MediaSource> getListOfMediaSource( FREArray array ) {
		List<MediaSource> result = new ArrayList<MediaSource>();

		try {
			for( long i = 0; i < array.getLength(); i++ ) {
				FREObject object = array.getObjectAt( i );
				MediaSource asset = new MediaSource();
				if( object instanceof FREBitmapData ) {
					asset.setBitmap( BitmapDataUtils.getBitmap( (FREBitmapData) object ) );
				} else {
					asset.setURL( getString( object ) );
				}
				result.add( asset );
			}
		} catch( Exception e ) {
